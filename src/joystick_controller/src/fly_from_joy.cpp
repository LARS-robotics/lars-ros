...
int main(int argc, char** argv)
{
	...
	joy_sub = node.subscribe("/joy", 10, joy_callback); //suscribe to the joystick message
	...
	while (ros::ok()){//} && new_msg) { //start the node when a messge comes in
		...
		joy_x=joy_x_;//map(joy_x_,-32767,32767,-1,1); //map the joy input (generally 10 bit to what the AR.Drone drivers expect)
		joy_y=joy_y_;//map(joy_y_,-32767,32767,-1,1);
		joy_z=joy_z_;//map(joy_z_,-32767,32767,-1,1);
