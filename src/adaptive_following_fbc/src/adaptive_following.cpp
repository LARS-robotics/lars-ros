...
double alpha = 5.0;	    // Radius Desired
...
int main(int argc, char **argv)
{
	...
	ros::Publisher dia_pub = n.advertise<adaptive_following::AdaptiveFollowingMsgs>(leader_robot_name + "/dia", 10);
	...
	ros::Publisher leader_vel = n.advertise<geometry_msgs::Twist>(leader_robot_name + "/cmd_vel", 10);
	...
	while (ros::ok())
	{
		...
		double h = pow(x[leader], 2) + pow(y[leader], 2) - pow(rd,2);
		double thetad = atan2(y[leader],x[leader]) + PI/2 + atan(alpha*h);
		
		double thetal_out = 0.0;
		double thetad_out = 0.0;
		double thetal_old = theta[leader];
		...
		if ( !firstExecution) {
			int n = thetal_loop;
			if( (thetal_old - theta[leader] )> PI/2 ){
	   			n = n+1;
	   		} else if((theta[leader] - thetal_old ) > PI/2 ){
				n = n-1;
			}
			thetal_old = theta[leader];
			thetal_out = theta[leader] + 2 * n * PI;
			thetal_loop = n;
		...
		// Send wheel velocities to driver
			...
			msg.linear.x = vl*1000;
			msg.linear.y = 0;
			msg.linear.z = 0;
			msg.angular.x = 0;
			msg.angular.y = 0;
			msg.angular.z = wl;
			leader_vel.publish(msg);
		} else {
			geometry_msgs::Twist msg;
			msg.linear.x = 0;
			msg.linear.y = 0;
