...
int main(int argc, char **argv)
{
	...
	if ( robot_name.compare("gizmo") == 0 ) {
		id = 0;
	} else if ( robot_name.compare("hal") == 0) {
		id = 1;
	} else if ( robot_name.compare("coyote") == 0) {
		id = 2;
	} else if ( robot_name.compare("road_runner") == 0) {
		id = 3;
	}
	...
	while (ros::ok())
	{
		...
		*/
		...
		//xcbar0 = 0.25*(x[0] + x[1] + x[2] + x[3]); 
		//ycbar0 = 0.25*(y[0] + y[1] + y[2] + y[3]);
