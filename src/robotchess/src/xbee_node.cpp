...
int main(int argc, char **argv)
{
	...
	ros::Subscriber sub = n.subscribe("torque", 1000, TorqueCallback);

	ros::Publisher encoder_pub = n.advertise<robotchess::Encoder>("encoder", 1000);
	...
	address.addr64_enabled = 1;ros::duration
	...
	while (ros::ok())
	{
        ...
        ROS_INFO_STREAM("I'm sending this message: " << msg[0]
                                                    << " " << msg[1]
                                                    << " " << msg[2]
                                                    << " " << msg[3]
                                                    << " " << msg[4]);
